/**
 * Reset all Prompt Wars Redis data (leaderboard, results, wins, attempts).
 * Usage: pnpm reset
 */
...
if (!skipConfirm) {
  ...
  const answer = await new Promise<string>((resolve) =>
    rl.question(
      `Delete ALL Prompt Wars data from ${url}? (type YES): `,
      resolve
    )
  )
  rl.close()
...
const PATTERNS = [
  'fsh:pw:leaderboard:data:*',
  'fsh:pw:leaderboard:ratelimit:*',
  'fsh:pw:result:*',
  'fsh:pw:win:*',
  'fsh:pw:attempts:*',
]

const EXACT_KEYS = ['fsh:pw:leaderboard']
...
async function main() {
  console.log('Resetting Prompt Wars Redis data...\n')
  ...
  console.log(
    '\nNOTE: Clear browser localStorage to reset client-side progress.' +
      "\n  → DevTools → Application → Local Storage → delete 'prompt-wars-session'"
  )
}
